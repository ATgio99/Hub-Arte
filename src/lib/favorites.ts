...
/** Azzera anche la copia in memoria, non solo il disco.
 *
 *  Uscendo da un account il codice svuotava `localStorage`, ma da quando la
 *  lista vive in memoria quella cancellazione non bastava piu': la copia in RAM
 *  restava, veniva riscritta su disco dalla scrittura differita, e all'accesso
 *  successivo finiva spinta sul cloud dell'altro account. E' cosi' che due
 *  profili diversi si sono ritrovati con gli stessi numeri. */
export function azzeraFavoritesLocali() {
  if (scritturaProgrammata) { clearTimeout(scritturaProgrammata); scritturaProgrammata = null; }
  memoria = { works: [], artists: [] };
  indice = { works: new Set(), artists: new Set() };
  scritturaNostra.clear();
  try {
