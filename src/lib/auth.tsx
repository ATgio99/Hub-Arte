...
import { azzeraFavoritesLocali } from "./favorites";
import { azzeraStudiedLocali } from "./studied";
...
// Chi era collegato l'ultima volta: serve ad accorgersi di un cambio di
// persona anche quando non si passa dal tasto «Esci».
let _ultimoUtente: string | null = null;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        const u = session?.user ?? null;
        // Se cambia la persona collegata, i dati locali del precedente non
        // devono sopravvivere nemmeno quando l'uscita non passa dal tasto
        // «Esci»: sessione scaduta, accesso da un altro dispositivo, o una
        // finestra ripristinata dal browser.
        const precedente = _ultimoUtente;
        if (u?.id && precedente && precedente !== u.id) {
          azzeraFavoritesLocali();
          azzeraStudiedLocali();
        }
        if (u?.id) _ultimoUtente = u.id;
        setUser(u);
        setIsAdmin(isAdminEmail(u?.email));
        setLoading(false);
  ...
  const signOut = useCallback(async () => {
    ...
    try {
      ...
      // Le liste vivono in memoria, non solo su disco: svuotare le chiavi di
      // localStorage non basta piu'. Senza questo, la copia in RAM sopravvive
      // all'uscita, viene riscritta su disco dalla scrittura differita, e
      // all'accesso successivo finisce spinta sul cloud dell'altro account.
      azzeraFavoritesLocali();
      azzeraStudiedLocali();
