...
async function loadDbOverrides(): Promise<Partial<Dataset>> {
  try {
    const [periodsRes, worksRes, artistsRes, techRes, termsRes, eventsRes, connsRes, hiddenRes] = await Promise.all([
      ...
      supabase.from("hidden_entities").select("id,table_name"),
    ]);
    // Salva gli hidden IDs in una variabile globale per il filtraggio
...
export function loadDataset(): Promise<Dataset> {
  ...
  _cache = (async () => {
    ...
    try {
      ...
      // Funzione helper: filtra via gli ID nascosti
