...
export function WorkGallery({ work }: { work: Work }) {
  ...
  return (
    <>
      <div className="opera-img card" style={{ position: "relative", overflow: "hidden", cursor: "pointer" }} onClick={() => setZoom(true)} role="button" tabIndex={0} aria-label="Clicca per ingrandire" onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setZoom(true); } }}>
      ...
      {zoom && (
        <div onClick={() => setZoom(false)} style={{ position: "fixed", inset: 0, zIndex: 10000, background: "rgba(0,0,0,0.92)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "zoom-out", padding: 24, animation: "fadeIn .2s" }}>
          <img src={images[idx]} alt={`${work.title} — immagine ${idx + 1} di ${n}`} style={{ maxWidth: "100%", maxHeight: "100%", objectFit: "contain", borderRadius: 4, boxShadow: "0 8px 40px rgba(0,0,0,0.5)" }} onClick={e => e.stopPropagation()} />
          <button onClick={() => setZoom(false)} aria-label="Chiudi" style={{ position: "fixed", top: 18, right: 18, zIndex: 10001, width: 44, height: 44, borderRadius: "50%", background: "rgba(255,255,255,0.15)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.25)", color: "#fff", fontSize: 22, lineHeight: 1, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
          {n > 1 && (
            <>
              ...
              <div style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", zIndex: 10001, color: "rgba(255,255,255,0.7)", fontSize: 14, fontFamily: "var(--font-display)", pointerEvents: "none" }}>{idx + 1} / {n} — {work.title}</div>
            </>
          )}
        </div>
      )}
