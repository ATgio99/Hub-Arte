...
import { STONES, type StoneDef } from "../lib/pages";
...
function SidebarBody({ collapsed, onToggleCollapse, isHome, onNavigate }: {
  ...
  // --- "Continua →" state: ultima opera/autore/rete/mappa/timeline visitata ---
  ...
  // Aggiorna i valori quando cambia la rotta (es. appena tornati su una pagina)
  useEffect(() => {
  ...
  // Ascolta l'evento "last-visited-changed" per aggiornare in tempo reale
  useEffect(() => {
    const onUpdate = () => {
      setLastOperaId(getLastOpera());
      setLastArtistaId(getLastArtista());
  ...
  // Etichette leggibili per ciascun "continua"
  ...
  // --- handleClick: doppio-click per navigare all'ultima visita salvata ---
  // Pattern: se c'è un valore salvato E non siamo già su quella pagina → vai lì.
  // Altrimenti → pulisci il salvato e vai alla home della sezione.
  // Per Rete: se siamo già su /grafo, dispatch di "atlante:rete-reset".
  const handleClick = (p: StoneDef, ev: React.MouseEvent) => {
    onNavigate?.();
    // Determina se è un doppio click (event.detail === 2)
    const isDouble = ev.detail === 2;
    if (!isDouble) return; // singolo click: lascia fare al Link il default
    ev.preventDefault();

    const dispatchChanged = () => window.dispatchEvent(new CustomEvent("atlante:last-visited-changed"));

    if (p.id === "opere") {
      if (lastOperaId && !loc.pathname.startsWith("/opera/")) {
        nav(`/opera/${lastOperaId}`);
      } else {
        clearLastOpera(); dispatchChanged(); nav(p.route);
      }
    } else if (p.id === "artisti") {
      if (lastArtistaId && !loc.pathname.startsWith("/artista/")) {
        nav(`/artista/${lastArtistaId}`);
      } else {
        clearLastArtista(); dispatchChanged(); nav(p.route);
      }
    } else if (p.id === "rete") {
      if (lastReteFocus) {
        if (loc.pathname === "/grafo") {
          // Già sul grafo: reset del focus
          window.dispatchEvent(new CustomEvent("atlante:rete-reset"));
        } else {
          nav("/grafo");
        }
      } else {
        clearLastRete(); dispatchChanged(); nav(p.route);
      }
    } else if (p.id === "mappa") {
      if (lastMappaCity && !loc.pathname.startsWith("/luogo/")) {
        nav(`/luogo/${encodeURIComponent(lastMappaCity)}`);
      } else {
        clearLastMappa(); dispatchChanged(); nav(p.route);
      }
    } else if (p.id === "timeline") {
      if (lastTimelineId && !loc.pathname.startsWith("/periodo/")) {
        nav(`/periodo/${lastTimelineId}`);
      } else {
        clearLastTimeline(); dispatchChanged(); nav(p.route);
      }
    }
  };
    ...
    <>
      ...
      <nav className="sbx-nav" data-testid="sbx-nav">
        {STONES.map((p) => {
          ...
          // Determina se questa sezione ha un'ultima visita salvata
          let hasLast = false;
          let lastLabel: string | null = null;
          if (p.id === "opere" && lastOperaTitle) { hasLast = true; lastLabel = lastOperaTitle; }
          else if (p.id === "artisti" && lastArtistaName) { hasLast = true; lastLabel = lastArtistaName; }
          else if (p.id === "rete" && lastReteLabel) { hasLast = true; lastLabel = lastReteLabel; }
          else if (p.id === "mappa" && lastMappaLabel) { hasLast = true; lastLabel = lastMappaLabel; }
          else if (p.id === "timeline" && lastTimelineLabel) { hasLast = true; lastLabel = lastTimelineLabel; }
          return (
            <Link key={p.id} to={p.route} onClick={(ev) => handleClick(p, ev)}
              className={`sbx-item ${active ? "active" : ""} ${hasLast ? "has-last" : ""}`}
              ...
              <span className="sbx-label">
                <b>{p.name}</b>
                {hasLast && lastLabel ? (
                  <span className="sbx-continue">
                    <span className="sbx-continue-label">Continua →</span>
                    <span className="sbx-continue-name">{lastLabel}</span>
                  </span>
                ) : (
                  <i>{p.desc}</i>
                )}
...
export default function Sidebar() {
  const loc = useLocation();
  // CHANGE 11: disabilita il tema scuro (on-home) della sidebar su tutte le pagine,
  // così la Landing ha la stessa sidebar chiara delle altre pagine.
  const isHome = false;
